...
This is an alternate version that creates a homogeneous spherical medium around a sink particle,
meant to do a Bondi-accretion test. By default, it uses default GIZMO units
...
newr = np.interp(int_mass, data_mass_in_r, data_r)
...
v[:, 2] *= vr / newr
# print chek data
print('mdot avg: %g std %g' % (np.mean(data_density * data_r * data_r * data_vr * np.pi * 4.0),
...
for z in [0.125, 0.5, 1.0, 2.0, 4.0]:
    ...
    t_rad_IC = (data_mass_in_r[ind2]
                * (4.0 / 3.0 * np.pi)
                * data_r[ind2] ** 3) / \
               (4.0 * np.pi * np.sum(data_r[ind]
                                     * data_r[ind]
                                     * data_vr[ind]
                                     * dm_ic[ind]))
...
if filename is None:
    filename = "rho%3.2g_" % (rho_gas * (mass_unit / (length_unit ** 3))) \
               + ("MBH%g_" % (mass_unit * M_BH) if M_BH > 0 else "") \
               + "R_over_Rsonic%g_Res%d_Rs_over_Rsonic%g" % (R_over_Rsonic, res_effective, Rs_over_Rsonic) \
               + ".hdf5"
...
if GMC_units:
    ...
    replacements = {"NAME": "../ICs/" + filename.replace(".hdf5", ""),
