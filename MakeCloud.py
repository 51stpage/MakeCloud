...
Options:                                                                       
   ...
   --minmode=<N>        Minimum populated turbulent wave number for Gaussian initial velocity field, in units of pi/R [default: 2]
...
def get_glass_coords(N_gas, glass_path):
    ...
    while len(x) * np.pi * 4 / 3 / 8 < N_gas:
        print("Need %d particles, have %d. Tessellating 8 copies of the glass file to get required particle number" % (
            N_gas * 8 / (4 * np.pi / 3), len(x)))
        x = np.concatenate(
            [x / 2 + i * np.array([0.5, 0, 0]) + j * np.array([0, 0.5, 0]) + k * np.array([0, 0, 0.5]) for i in range(2)
...
if filename is None:
    filename = "M%3.2g_" % (M_gas) + ("MBH%g_" % (M_BH) if M_BH > 0 else "") \
    ...
    # n_crit based on assumption that dm=M_jeans, meaning that densest is still resolved by NJ particles
    ...
    replacements = {"NAME": filename.replace(".hdf5", ""),
                    "DTSNAP": tff / nsnap,
                    "MAXTIMESTEP": tff / (nsnap * 2),
                    "SOFTENING": softening,
                    "GASSOFT": 2.0e-8,
                    "TMAX": tff * tmax,
                    "RHOMAX": ncrit,
                    "BOXSIZE": boxsize / length_unit,
                    "OUTFOLDER": "output",
                    ...
                    "TURBFREQ": tcross / 20,
                    "TURB_KMIN": int(100 * 2 * np.pi / L) / 100.,
                    "TURB_KMAX": int(100 * 4 * np.pi / (L) + 1) / 100.,
                    "TURB_SIGMA": vrms,
    ...
    if makecylinder:
        ...
        vrms_cyl = (
                           2 * G * M_gas / L_cyl) ** 0.5 / v_unit * turbulence ** 0.5  # the potential is different for a cylinder than for a sphere, so we need to rescale vrms to get the right alpha, using E_grav_cyl = -GM**2/L
        ...
        replacements_cyl["TURB_KMIN"] = int(100 * 2 * np.pi / R_cyl) / 100.;
        replacements_cyl["TURB_KMAX"] = int(100 * 4 * np.pi / (R_cyl) + 1) / 100.;
        paramsfile = str(open(os.path.realpath(__file__).replace("MakeCloud.py", "params.txt"), 'r').read())
...
u = np.ones_like(mgas) * (
        200 / v_unit) ** 2  # start with specific internal energy of (200m/s)^2, this is overwritten unless starting with restart flag 2###### #0.101/2.0 #/2 needed because it is molecular
...
if warmgas:
    ...
    if derefinement:
        ...
        x0 = x0[
             N_warm:]  # now we take the ones outside the initial sphere and map them to a n(R) ~ R^-3 profile so that we get constant number of cells per log radius interval
        r0 = r0[N_warm:]
...
if sinkbox:
    ...
    m_max = sinkbox
    if sinkbox < m_min: print(
        "Maximum sink mass is less than the minimum of 10 times the gas mass resolution. Increase resolution or implement a different sampling scheme.")

    N_sinks = int(round(m_max / m_min))

    m_sinks = (1. / m_min - (1 / m_min - 1 / m_max) * np.random.rand(
        N_sinks)) ** -1.  # randomly sample from M^-2 mass function between M_max and M_min
    x_sinks = np.random.rand(N_sinks, 3) * boxsize  # random coordinates
    v_sinks = np.random.normal(size=(N_sinks, 3)) * (
            np.sum(v ** 2, axis=1).mean() / 3) ** 0.5  # random velocities equal to RMS gas velocity
...
if M_BH > 0:
    ...
    F["PartType5"].create_dataset("Coordinates",
                                  data=np.array([1., 1., 1.]) * boxsize / 2 / length_unit)  # at the center
    ...
    if (central_star or central_SN):
        if central_star:
            ...
            F["PartType5"].create_dataset("ProtoStellarStage", data=np.array([5], dtype=np.int32),
                                          dtype=np.int32)  # starts as ZAMS star
        else:
            print("Assuming central sink is a ZAMS star about to go supernova")
            F["PartType5"].create_dataset("ProtoStellarStage", data=np.array([6], dtype=np.int32),
                                          dtype=np.int32)  # starts as ZAMS star going SN
        # Set guess for ZAMS stellar radius, will be overwritten
        if ((M_BH) > 1.0):
            R_ZAMS = (M_BH) ** 0.57
        else:
            R_ZAMS = (M_BH) ** 0.8
        F["PartType5"].create_dataset("ProtoStellarRadius_inSolar",
                                      data=np.array([R_ZAMS]))  # Sinkradius set to softening
        ...
        F["PartType5"].create_dataset("ProtoStellarStage", data=np.array([0], dtype=np.int),
                                      dtype=np.int32)  # starts as pre-collapse
